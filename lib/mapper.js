...
exports.identify = function(msg, settings){
  ...
  var defaultMethod = settings.default || 'flatten';
  ...
  company = formatCompany(company, blacklisted); // returns array
  ...
  ret.custom_attributes = formatNested(ret.custom_attributes, blacklisted, defaultMethod);
...
exports.track = function(track, settings){
  ...
  var properties = formatNested(formatMetadata(track.properties()), blacklisted, defaultMethod);
  if (revenue) {
    var revenueData = {
...
exports.group = function(msg, settings) {
  ...
  var companyData = reject({
    ...
    custom_attributes: formatNested(customTraits, blacklisted, defaultMethod)
  });
 ...
 *
 * @param {Object} obj
 * @param {Object} blacklisted
 * @param {String} opts
 * @return {Object} ret
 * @api private
 */

function formatNested(obj, blacklisted, opts){
  return foldl(function(attrs, value, key){
    ...
    if (trait === 'stringify') {
      ...
      // fallback to default based on setting
      if (opts === 'stringify') attrs[key] = JSON.stringify(value);
      if (opts === 'flatten') attrs = extend(flatten(pair), attrs);
    }
