...
function formatNested(obj, settings){
  ...
  return foldl(function(attrs, value, key){
    each(function(mappedProp){
      if (key === mappedProp.key) {
        // what did want?
        // if drop, continue to next property
        // if stringify, JSON.stringify that shit (value) = attrs[key]
        // return out, move out to property
      } else {
        // {
        //   foo: 'bar',
        //   nested: {
        ...
        //   }
        // }
        // flatten({}.nested) = { nested_a_b: 'hooooooo' } 
        // merge it to attrs use `extend(original, what to merge)`
      }
    }, blacklisted);
