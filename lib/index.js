...
Intercom.prototype.track = function(event, fn){
  ...
  // Adding 'events' so we can still lock separately from locking in `.identify()` calls
  var key = [this.settings.appId, 'events', id].join(':');
  var self = this;
  ...
  return this
    ...
    .end(self.handle(function(err, res){
      ...
      self.unlock(key, function(){
        ...
        // Setting expiration to 14.75 mins since Intercom keeps
        // jobs open for 15 minutes, we want to add a buffer for
        // more reliability
        ...
        self.redis().set(
          ...
          function(err, ok){
            ...
            console.log('ok', ok);
            self.redis().get(jobKey, function(jobId){
