...
Intercom.prototype.identify = function(identify, fn){
  ...
  var blacklisted = this.settings.blacklisted;
  var defaultMethod = this.settings.defaultMethod;
  ...
  this.lock(key, function(err){
    ...
    return self
      ...
      .type('json')
      .accept('json')
      .send(mapper.identify(identify, {collectContext: collectContext, blacklisted: blacklisted, defaultMethod: defaultMethod}))
...
Intercom.prototype.group = function(group, fn){
  ...
  return this
    ...
    .type('json')
    .accept('json')
    .send(mapper.group(group, { blacklisted: blacklisted, defaultMethod: defaultMethod }))
...
Intercom.prototype.track = function(track, fn) {
  var blacklisted = this.settings.blacklisted;
  var defaultMethod = this.settings.defaultMethod;
  var richLinkProperties = this.settings.richLinkProperties;
  ...
  return this
    ...
    .type('json')
    .accept('json')
    .send(mapper.track(track, { blacklisted: blacklisted, defaultMethod: defaultMethod, richLinkProperties: richLinkProperties }))
    .end(this.handle(fn));
};
